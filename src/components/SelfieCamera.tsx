...
export default function SelfieCamera({ onCapture }: SelfieCameraProps) {
  ...
  return (
    <div className="w-full max-w-lg mx-auto glass-morphism rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-2xl relative overflow-hidden flex flex-col items-center">
      ...
      {/* Error message */}
      {error && (
        <p className="text-[10px] sm:text-xs text-red-400/80 mt-2 text-center leading-relaxed bg-red-950/20 p-2 rounded-lg border border-red-900/30 w-full">
          {error}
        </p>
      )}
      ...
      <div className="w-full mt-3.5 sm:mt-4 space-y-2 sm:space-y-2.5">
        {!capturedPhoto ? (
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5">
            ...
            <button
              ...
              disabled={loading}
              className="py-2 sm:py-2.5 px-1 rounded-lg sm:rounded-xl border border-gold-300/20 hover:border-gold-300/50 bg-gold-400/5 text-gold-200 transition-all duration-300 flex flex-col items-center justify-center gap-0.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Camera className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
              <span className="text-[10px] sm:text-xs font-sans whitespace-nowrap">自拍模式</span>
            </button>
            ...
            <button
              ...
              disabled={loading}
              className="py-2 sm:py-2.5 px-1 rounded-lg sm:rounded-xl border border-cyan-400/20 hover:border-cyan-400/50 bg-cyan-400/5 text-cyan-200 transition-all duration-300 flex flex-col items-center justify-center gap-0.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Aperture className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
              <span className="text-[10px] sm:text-xs font-sans whitespace-nowrap">拍照模式</span>
            </button>
            ...
            <button
              ...
              disabled={loading}
              className="py-2 sm:py-2.5 px-1 rounded-lg sm:rounded-xl border border-pink-500/20 hover:border-pink-500/50 bg-pink-500/5 text-pink-200 transition-all duration-300 flex flex-col items-center justify-center gap-0.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Upload className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
              <span className="text-[10px] sm:text-xs font-sans whitespace-nowrap">相簿上傳</span>
            </button>
          </div>
        ) : (
          <div className="space-y-1.5 sm:space-y-2.5">
            ...
            <button
              onClick={retakePhoto}
              className="w-full py-2 sm:py-2.5 bg-white/5 hover:bg-white/10 text-gray-300 text-xs rounded-lg sm:rounded-xl transition-all duration-300 flex items-center justify-center gap-1.5 cursor-pointer border border-white/5"
            >
